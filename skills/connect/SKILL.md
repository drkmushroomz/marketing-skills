---
name: connect
description: First-run setup — verify all MCP connections and personalize your tools
...
7. **Skill Personalization**: After all connections are verified, walk the user through personalizing their tools.
   ...
   **Q10 — Dev activity** (`include_dev_activity`, default: `false`):
   > "Do you write code? If so, I can include your git commits in your daily recap."
   >
   > - **Yes** — include code changes
   > - **No** (default) — skip dev activity

   **Q11 — Focus** (`focus_areas`, default: `[]`):
   ...
   **Dev Activity (only if Q10 = yes) → `skills.dev-activity.preferences`**

   **Q14 — Repos** (`include_repos`, default: `"all"`):
   > "I'll scan ~/Projects for your git repos. Want me to include all of them, or just specific ones?"
   >
   > - **All** (default) — scan everything in ~/Projects
   > - **Specific** — let me list the ones to include

   **Q15 — Exclude repos** (`exclude_repos`, default: `[]`):
   > "Any repos to skip? Like personal projects or forks you don't need tracked."

   ---

   **Meeting Transcripts → `skills.fetch-daily-transcripts.preferences`**

   **Q16 — Granola** (`use_granola`, default: `false`):
   > "Do you use Granola for meeting notes? If yes, I'll pull your transcripts from there."
   >
   > - **Yes** — use Granola
   > - **No** (default) — skip Granola

   **Q17 — Google Meet** (`use_google_transcripts`, default: `true`):
   ...
   After all questions, set `skills.<slug>.onboarded = true` for each section: `_shared`, `email-update`, `slack-summary`, `daily-recap`, `start-of-day`, `dev-activity` (if applicable), `fetch-daily-transcripts`.
